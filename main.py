...
def extract_graph(req: ExtractRequest):
    try:
        ...
        # Using beta.chat.completions.parse forces the LLM to output the exact Pydantic schema
...
def community_summary(req: CommunityRequest):
    try:
        ...
        response = client.beta.chat.completions.parse(
            ...
            temperature=0.3 # Slight creativity allowed for summarization phrasing
        )
        
        parsed_response = response.choices[0].message.parsed
        # Hardcode the community_id to ensure it matches the request perfectly, overriding any LLM typos
